  ...
  export function CreateTree(): TreeItem[] {
    ...
    for (let index = 0; index < list.length; index++) {
      ...
      if(element)  {
        ...
        if(opG[str1] != undefined) {
          ...
          if(element.prG in opG[str1]){
            opG[str1][element.prG]?.push({label: element.label, key: cntr++, content: element, icon: 'cancel'});
          }
        }
      }
    }
    for (const [key, value] of Object.entries(opG)) {
      ...
      for (const [k1, v1] of Object.entries(value)) {
        ...
        prGlist.push({selectable: false, label: k1, children: v1, key: cntr++, icon: 'search'})
      }if( key != ''){
